...
void Wrp_InitSDL(const char *windowName) {
	...
	window = SDL_CreateWindow(windowName, SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, width, height, SDL_WINDOW_SHOWN | SDL_WINDOW_RESIZABLE/* | SDL_WINDOW_MAXIMIZED*/);
	rend = SDL_CreateRenderer(window, -1, SDL_RENDERER_ACCELERATED | SDL_RENDERER_PRESENTVSYNC);
	
	TTF_Init();
	
	texture_array = (SDL_Texture**)malloc(1);
...
void Wrp_LoadTexture(const char *filePath) {
	...
	texture_array = (SDL_Texture**)realloc(texture_array, ++texArrayLen*sizeof(void*));
